...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Volumes/home/DanaSmall-CC1622-Drive/Small_Lab/resources, equipment and manuals/Resources/Psychopy Tasks/digit_symbol_coding/digit_symbol_coding_lastrun.py',
...
while continueRoutine:
    ...
    if practice_resp_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(practice_resp_2.clock.reset)  # t=0 on next screen flip
    ...
    if practice_resp_2.status == STARTED and not waitOnFlip:
        ...
        _practice_resp_2_allKeys.extend(theseKeys)
        if len(_practice_resp_2_allKeys):
            practice_resp_2.keys = _practice_resp_2_allKeys[-1].name  # just the last key pressed
            practice_resp_2.rt = _practice_resp_2_allKeys[-1].rt
...
while continueRoutine:
    ...
    if practice_response_3.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(practice_response_3.clock.reset)  # t=0 on next screen flip
    ...
    if practice_response_3.status == STARTED and not waitOnFlip:
        ...
        _practice_response_3_allKeys.extend(theseKeys)
        if len(_practice_response_3_allKeys):
            practice_response_3.keys = _practice_response_3_allKeys[-1].name  # just the last key pressed
            practice_response_3.rt = _practice_response_3_allKeys[-1].rt
